...
import MembresiaCrearUsuario from "./MembresiaCrearUsuario"
import MembresiaUsuariosDetalle from "./MembresiaUsuariosDetalle"
import MembresiaEditarUsuario from "./MembresiaEditarUsuario"
import MembresiaProductosDetalle from "./MembresiaProductosDetalle"
import UserContext from "../providers/sesion/UserContext"
import Swal from "sweetalert2"

export default function MembresiaConfiguracion(){
    const {id} = useParams()
    ...
    const [usuMembresia, setUsuMembresia] = useState([])
    ...
    const axiosEncabezado = async () => {
        const ruta=url+"membresiaEncabezado/"+id
    ...
    const filtrActual=()=>{
        const filtrados=usuarios?.filter((elemento)=>{
            if(elemento.membreEncaId===seleccionado.id){
                return elemento
            }
        });
        setUsuMembresia(filtrados);
    }

    // Seleccionar Productos cargados
    const axiosProductosCarga = async()=>{
        const rutapro=url+"membresiaProducto/"+id
    ...
    const elegir = async (uni)=>{        
        ...
        const ruta=url+"membresiaProducto/"+id+"/"+uni
        ...
        await axios.get(ruta)
            .then((res)=>{                 
                if(res.data===null){        
                    ...
                    Swal.fire(
                        '¡Error!',
                        'Este producto ya esta asignado a esta membresia.',
                        'error'
                    )                
                }
            })
    ...
    const asignarlo = async(uni)=>{
        const ruta=url+"membresiaProducto"
        
        const { value: porcentaje } = await Swal.fire({
            title: 'Registre el porcentaje de descuento para este producto',
            input: 'text',
            ...
            <div className="container text-center">
                <div className="alert alert-primary" role="alert">
                    <h5 className="modal-title" id="productos">
                        ...
                        <NavLink to="/membresia"><button type="button" className="btn btn-primary btn-sm">volver</button></NavLink>
                    </h5>
                </div>
                {
                                ...
                                <table className="table table-success table-hover table-bordered table-responsive table-striped"> 
                                    ...
                                    <tbody>                        
                                        {usuarios.map((usuario, index)=>(                                        
                                            <MembresiaUsuariosDetalle usuario={usuario} key={index} setEditUsuario={setEditUsuario} id={id}/>
                                ...
                                <div className="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false"  aria-labelledby="staticBackdropLabel" aria-hidden="true">
                                    <div className="modal-dialog modal-dialog-scrollable">
                                        <div className="modal-content">
                                            ...
                                            <div className="modal-body">
                                                <MembresiaEditarUsuario editUsuario={editUsuario} id={id}/>      
                                            </div>                                           
                                        </div>
                                    </div>
                                </div> 
                                ...
                                <table className="table table-success table-hover table-bordered table-responsive table-striped"> 
                                    ...
                                    <tbody>                        
                                        {producSel.map((produc, index)=>(                                        
                                            <MembresiaProductosDetalle produc={produc} key={index} id={id}/>
                                        )).slice(firstIndex,lastIndex)}                        
                                    </tbody>
                                </table>
                            </>
