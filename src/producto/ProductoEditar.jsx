...
export default function ProductoEditar({editProducto}) {
...
const onSubmit = async (productoInfo) => {    
  ...
  .then((response)=>{
    ...
    if(response.data.length>0 && response.data[0].id !== productoInfo.id){
      Swal.fire(
        '¡IMPORTANTE!',
        `Ya existe un producto con este código de barras nnnnnnn: ${productoInfo.code}`,
        'error'
      )
    }else{
      actualizaProducto(productoInfo)
    }
  }).catch((error)=>{
    console.log(error)
  })    
...
const actualizaProducto = async (productoInfo) => {

  const ruta = url+"producto/"+editProducto.id;
  
      axios.put(ruta, productoInfo).then((response) =>{
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="modal-footer">
        ...
        <button type="submit" className="btn btn-info">Modificar Producto</button>
      </div>
